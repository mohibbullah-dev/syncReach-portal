        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {list.map((p) => (
            <article
              key={p.id}
              className={cn(
                "group relative flex flex-col overflow-hidden rounded-[1.35rem] border bg-gradient-to-b from-white to-slate-50/40 p-6 transition duration-300 hover:-translate-y-0.5",
            ...
            >
              ...
              <ul className="relative mt-5 space-y-2 border-t border-slate-100 pt-4 text-sm text-slate-600">
                {p.features.slice(0, 4).map((f) => (
                  <li key={f} className="flex items-start gap-2">
                    <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-[#0061FF]" />
              ...
              <Button
                variant="outline"
                className="relative mt-5 w-full rounded-xl border-slate-200 group-hover:border-[#0061FF]/40 group-hover:bg-[#F4F8FF]"
                onClick={() => {
                  setEditing(p);
                  setOpen(true);
                }}
